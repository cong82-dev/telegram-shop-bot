...
const orderService = {
  ...
  confirmAndDeliver(orderId) {
    ...
    if (!order) return { success: false, error: "Order does not exist" };
    if (order.status !== "pending")
      return { success: false, error: "Order has already been processed" };

    // Get available stock
    const stock = productService.getAvailableStock(
      order.product_id,
      order.quantity,
    );
    if (stock.length < order.quantity) {
      return {
        success: false,
        error: `Not enough stock. Only ${stock.length} items left.`,
      };
    }
  ...
  getStats() {
    const totalOrders = db
      .prepare("SELECT COUNT(*) as c FROM orders WHERE status = 'delivered'")
      .get().c;
    const totalRevenue = db
      .prepare(
        "SELECT COALESCE(SUM(total_price), 0) as s FROM orders WHERE status = 'delivered'",
      )
      .get().s;
    const pendingOrders = db
      ...
      .get().c;
    const totalStock = db
      .prepare("SELECT COUNT(*) as c FROM stock WHERE is_sold = 0")
      .get().c;
